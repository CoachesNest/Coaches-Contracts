...
This document explains how to manage multi-network configuration for deploying Coaches contracts to Soroban.
...
cargo run -p coaches-tools -- network show
...
```bash
cargo run -p coaches-tools -- network list
...
cargo run -p coaches-tools -- deploy \
  --network mainnet \
  --wasm target/wasm32-unknown-unknown/release/coaches_core.wasm
```

### 4. CLI: Show Configuration as JSON

```bash
cargo run -p coaches-tools -- config --json
...
```bash
# Test on testnet
SOROBAN_NETWORK=testnet cargo run -p coaches-tools -- network show

# Deploy to mainnet
SOROBAN_NETWORK=mainnet cargo run -p coaches-tools -- deploy \
  --wasm target/wasm32-unknown-unknown/release/coaches_core.wasm
```
...
```bash
cargo run -p coaches-tools -- config --validate
...
jobs:
  deploy:
    ...
    steps:
      ...
      - run: cargo run -p coaches-tools -- config --validate
      - run: cargo build -p coaches-core --target wasm32-unknown-unknown --release
      - run: cargo run -p coaches-tools -- deploy --wasm target/wasm32-unknown-unknown/release/coaches_core.wasm
```
...
config:
	cargo run -p coaches-tools -- config

deploy-testnet:
	SOROBAN_NETWORK=testnet cargo run -p coaches-tools -- deploy --wasm target/wasm32-unknown-unknown/release/coaches_core.wasm

deploy-mainnet:
	SOROBAN_NETWORK=mainnet cargo run -p coaches-tools -- deploy --wasm target/wasm32-unknown-unknown/release/coaches_core.wasm
```
