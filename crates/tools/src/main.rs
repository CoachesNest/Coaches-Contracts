...
#[derive(Parser)]
#[command(name = "coaches")]
#[command(about = "Coaches CLI tools for contract deployment and management")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    match cli.command {
        Commands::Network { action } => match action {
            ...
            Some(NetworkAction::List) => {
                ...
                println!("  cargo run -p coaches-tools -- network show");
                Ok(())
            }
            None => {
                let config = Config::load()?;
