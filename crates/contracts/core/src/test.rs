...
use super::*;
use crate::{CoachesContract, CoachesContractClient, DEFAULT_DISPUTE_WINDOW_SECONDS, DEFAULT_DISPUTE_WINDOW_LEDGERS, DISPUTE_WINDOW_MIN_LEDGERS, DISPUTE_WINDOW_MAX_LEDGERS};
...
fn setup_with_admin() -> (
    ...
    let contract = CoachesContractClient::new(&env, &contract_id);
...
fn setup_env() -> (Env, CoachesContractClient, Address, Address) {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register_contract(None, CoachesContract);
