...
pub struct Session {
    ...
    //    Example: SHA256(CSPRNG bytes) ΓåÆ truncate or use full hash as ID
...
mod tests {
    ...
    fn test_set_platform_fee_bps_event_reflects_successive_updates() {
        ...
        // First update: 0 ΓåÆ 500
        client.set_platform_fee_bps(&500);
        assert_eq!(client.get_platform_fee_bps(), 500);

        // Second update: 500 ΓåÆ 1000
        client.set_platform_fee_bps(&PLATFORM_FEE_MAX_BPS);
        assert_eq!(client.get_platform_fee_bps(), PLATFORM_FEE_MAX_BPS);

        // Third update: 1000 ΓåÆ 0
        client.set_platform_fee_bps(&0);
        assert_eq!(client.get_platform_fee_bps(), 0);

        // events[0] = Initialized, [1] = PlatformFeeUpdated(0ΓåÆ500),
        // [2] = PlatformFeeUpdated(500ΓåÆ1000), [3] = PlatformFeeUpdated(1000ΓåÆ0)
        assert_eq!(
            env.events().all(),
            vec![
                &env,
    ...
    fn test_funds_released_net_amount_correct() {
        ...
        // 0% fee ΓÇö mentor receives full amount
        assert_eq!(token_client.balance(&payee), amount);
    }
    ...
    // ΓöÇΓöÇ Input validation tests ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    fn setup(env: &Env) -> (Address, CoachesContractClient) {
        env.mock_all_auths_allowing_non_root_auth();
        let id = env.register_contract(None, CoachesContract);
        let client = CoachesContractClient::new(env, &id);
        let admin = Address::generate(env);
        let treasury = Address::generate(env);
        client.init(&admin, &100, &treasury, &DEFAULT_DISPUTE_WINDOW_SECONDS);
        (id, client)
