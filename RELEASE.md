...
This document explains how to cut releases for Coaches Contract and verify artifact integrity.
...
```bash
git clone https://github.com/<owner>/<repo>.git coaches-contract
cd coaches-contract
...
```bash
wasm-tools strip \
  target/wasm32-unknown-unknown/release/coaches_core.wasm \
  -o coaches_core_stripped.wasm

mv coaches_core_stripped.wasm \
   target/wasm32-unknown-unknown/release/coaches_core.wasm
```
...
wasm-opt -Oz \
  target/wasm32-unknown-unknown/release/coaches_core.wasm \
  -o coaches_core_opt.wasm

mv coaches_core_opt.wasm \
   target/wasm32-unknown-unknown/release/coaches_core.wasm
```
...
EXPECTED="a1b2c3d4..."
COMPUTED=$(sha256sum target/wasm32-unknown-unknown/release/coaches_core.wasm | cut -d' ' -f1)
...
# Clone and checkout
REPO_DIR="/tmp/coaches-verify-$VERSION"
rm -rf "$REPO_DIR"
...
4. Check you're using `--locked`: `cargo +stable build -p coaches-core --target wasm32-unknown-unknown --release --locked`
